...
/// A sentence, word by word, with the punctuation the patterns depend on.
enum Words {
    struct Word: Sendable, Hashable {
        /// As printed, less the punctuation around it: "Reen".
        var display: String
        ...
        /// tells an appositive from a compound name.
        var followedByComma: Bool
    }

    static func split(_ sentence: String) -> [Word] {
        var out: [Word] = []
        for chunk in sentence.split(whereSeparator: \.isWhitespace) {
